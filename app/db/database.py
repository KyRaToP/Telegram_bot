...
async def init_db() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, registered_at TEXT)"
        )
        await db.execute(
            "CREATE TABLE IF NOT EXISTS tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, title TEXT, description TEXT, due_time TEXT, is_completed INTEGER DEFAULT 0, category TEXT DEFAULT 'Без категории')"
        )
        await db.execute(
...
async def add_task(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO tasks (user_id, title, description, due_time, is_completed, category) VALUES (?, ?, ?, ?, 0, ?)",
...
async def get_user_tasks(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if category:
            cursor = await db.execute(
                "SELECT * FROM tasks WHERE user_id = ? AND is_completed != 1 AND category = ? ORDER BY due_time ASC",
                (user_id, category),
            )
        else:
            cursor = await db.execute(
                "SELECT * FROM tasks WHERE user_id = ? AND is_completed != 1 ORDER BY due_time ASC",
                (user_id,),
            )
...
async def get_completed_tasks(user_id: int) -> List[Dict[str, Any]]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            "SELECT * FROM tasks WHERE user_id = ? AND is_completed == 1 ORDER BY due_time DESC",
            (user_id,),
        )
        return [dict(row) for row in await cursor.fetchall()]
